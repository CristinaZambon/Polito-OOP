...
public class Delivery {
	...
	public Map<String,List<String>> getDishesByPrice(float minPrice, float maxPrice) {
	    return restaurants.values().stream()
	            .flatMap( r->r.getDishes().stream() ) //creo stream of all dishes
	...
	public List<String> getDishesForRestaurant(String restaurantName) {
		...
		if( r == null ) return new ArrayList<>(); //return empty list
...
//		return res;
		...
		.collect(Collectors.groupingBy(	Restaurant::getCategory,	//stream of orders per restaurant r
										Collectors.flatMapping( r -> orders.stream().filter(o -> r == o.getRestaurant()),
																Collectors.counting()
						)));
	}
